<div class="modal-dialog modal-open">
    <div class="modal-content rounded-bottom">
        <div class="modal-header">
            <h4 class="modal-title pull-left">Update: <?= $updateCity->cityName ?></h4>
            <button type="button" class="close pull-right" data-dismiss="modal">&times;</button>
        </div>
        <div class="modal-body">
            <div class='panel panel-body'>
                <form novalidate class="form-group" method="post"
                      action="<?= sysUrl('updateCity/' . $updateCity->cityID) ?>">

                    <div class="form-group">
                        <label for="cityName">City Name</label>
                        <input type="text" class="form-control" name="cityName" id="cityName"
                               value="<?= $updateCity->cityName ?>">
                        <span id="cityName_result"></span>
                    </div>
                    <div class="form-group text-center">
                        <button type="submit" class="btn btn-primary" id="addCityBtn">Update City</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    $(document).ready(function () {
        $('#cityName').change(function () {
            ...
            if (cityName != '') {
                $.ajax({
                    url: "<?=sysUrl("checkCityAvalibility")?>",
                    method: "POST",
                    data: {cityName},
